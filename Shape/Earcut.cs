...
{
  public static int[] Tessellate(float[] data, int[] holeIndices)
  {
    return Tessellate(data.ToList(), holeIndices.ToList()).ToArray();
  }
  ...
  static Node CureLocalIntersections(Node start, IList<int> triangles)
  {
    ...
    do
    {
      ...
      if (
        !Equals(a, b) && Intersects(a, p, p.next, b) && LocallyInside(a, b) && LocallyInside(b, a)
      )
      {
        triangles.Add(a.i / 2);
        triangles.Add(p.i / 2);
